...
// router.get('/',userController.getMyHabits, userController.checkProgress, (req, res, next) => {
...
//userController.verifyUser or cookieController.verifyToken
router.get('/',cookieController.token,userController.getMyHabits,  (req, res, next) => {
  // first invoke controller to retrieve list of all habits at current date for current user
  // return list
  const user=res.locals.user;
  const myHabits=res.locals.myHabits;
